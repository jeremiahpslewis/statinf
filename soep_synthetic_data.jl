...
- Simple mixing function for age and education -> wage
- Each household has target income
- Constraints for achieving this target

- What appears to be a gender based effect is a (dis-)associative matching result
"""

# ╔═╡ 65ed3871-756b-45f8-9ba2-e06fbc3bf2f6
cm"# SOEP Basic Data for Stata Data Visualization"

# ╔═╡ 30483a0d-f9f4-418e-877d-f500d7062049
mutable struct SOEPSyntheticStataData
	...
	mean_wage_edu_age_m::Vector
	mean_wage_edu_age_w::Vector

	hourly_wage_m::Vector
	hourly_wage_w::Vector

	# Real term difference between man wages and woman wages
	wage_gap::Vector
	pct_child_care_responsibility_m::Vector
	childcare_external_hours::Vector
	
	child_care_hours_m::Vector
	child_care_hours_w::Vector
	
	desired_hours_worked_m::Vector
	desired_hours_worked_w::Vector

	actual_hours_worked_m::Vector
	actual_hours_worked_w::Vector

end

# ╔═╡ 3fa0a098-81da-440a-bec6-cf0933cf108f
# Create empty 'placeholder' objects for storing simulated data
function simulate_(model, n_households)
	params = Dict(:n_households => n_households)

	n_objs = 17
	_vectors = []
	for i in 1:n_objs
	# Household Level
		push!(_vectors, Vector{Union{Float64,Missing}}(missing, n_households))
	end
...
# Specify the data generating process as a set of draws from distributions and truncation steps
m1 = @model (params,
		age_m,
		age_w,
		...
		wage_gap,
		pct_child_care_responsibility_m,
		childcare_external_hours,

		child_care_hours_m,
		child_care_hours_w,

		desired_hours_worked_m,
		desired_hours_worked_w,
	...
	@inbounds for i = 1:params[:n_households]
		...
		age_w[i] ~ truncated(Distributions.Normal(0.9, 0.2); lower = 0.7, upper = 1.3)
		age_w[i] = age_m[i] * age_w[i] # Specify woman's age as function of man + noise
		age_m[i] = clamp(age_m[i], 20, 55)
		age_w[i] = clamp(age_w[i], 20, 55)

		education_m[i] ~ truncated(Distributions.Normal(1.5, 0.5); lower=0, upper=3)
		education_w[i] ~ Distributions.Normal(0.9, 0.3)
		# Women's education is, on average, somewhat lower than men in the sample
		# Ensure stays within range
		education_w[i] = clamp(education_m[i] * education_w[i],  0, 3)

		# Hack to ensure variables are tracked:
		mean_wage_edu_age_m[i] ~ Distributions.Uniform(0,1)
	...
	@inbounds for i = 1:params[:n_households]
		...
		hourly_wage_w[i] ~ Distributions.Normal(mean_wage_edu_age_w[i], sd_wage__by_edu_age)

		# Hack to ensure variables are tracked:
		wage_gap[i] ~ Distributions.Uniform(0,1)
		actual_hours_worked_m[i] ~ Distributions.Uniform(0,1)
		...
		# Determine percentage primary child care responsibilities based on gap between wages
		pct_child_care_responsibility_m[i] = 1 - logistic(4 * wage_gap[i] / mean([hourly_wage_m[i], hourly_wage_w[i]]))

		# gap in wages is the percentage of avg household hourly wages, mapped through a logistic to a probability; 0 -> man has no child care responsibility, 1 -> man has 100% child care responsibility

		# Assume ideal working hours gender neutral, average of observed hours amongst genders
		avg_obs_hours = mean([16.54, 38.27])
		avg_obs_hours_sd = mean([15.09, 13.10])
		
		desired_hours_worked_m[i] ~ Distributions.Normal(avg_obs_hours, 5)
		desired_hours_worked_w[i] ~ Distributions.Normal(avg_obs_hours, 5)
...
function toDataFrame(tilde_data)
	df = DataFrame()
	for field_ in fieldnames(typeof(tilde_data))
		if field_ != :params
			df[!, field_] = getproperty(tilde_data, field_)
		end
	end	
	return df
end
...
# ╔═╡ a1e01ee4-935f-49f2-a3b1-d512d30d6cc0
data = toDataFrame(simulate_(m1, 2003))

# ╔═╡ 0a763e17-e679-4775-ae51-d8f3ee979ac6
CSV.write("stata_input.csv", data)

# ╔═╡ 5b947b20-8dcf-4cd4-8a8c-b5f81676efd8
mean(data[!, :age_m])
...
design_matrix = vcat(w_design_matrix, m_design_matrix)

# ╔═╡ ef185ea6-be92-43c6-aea4-60df5fc30b97
probit1 = glm(@formula(primary_child_care_responsibility ~ age + education + gender_w ), design_matrix, Distributions.Binomial(), LogitLink())

# ╔═╡ ceadd7d2-5878-4ef6-a022-3b7503963d62
probit2 = glm(@formula(primary_child_care_responsibility ~ age + education + gender_w + hourly_wage), design_matrix, Distributions.Binomial(), LogitLink())

# ╔═╡ 91784666-1a93-4e7f-bd19-c24f4a4f91de
probit3 = glm(@formula(primary_child_care_responsibility ~ age + education + gender_w + hourly_wage + wage_gap), design_matrix, Distributions.Binomial(), LogitLink())

# ╔═╡ 21c62a02-1e35-4119-8b25-8d9921659b53
probit4 = glm(@formula(is_restricted ~ age + education + gender_w +
	childcare_coverage_available),
	design_matrix, Distributions.Binomial(), LogitLink())

# ╔═╡ cb308118-6a25-4942-9475-ae30c174fcf3
probit5 = glm(@formula(is_restricted ~ age + education + gender_w + childcare_coverage_available + hourly_wage), design_matrix, Distributions.Binomial(), LogitLink())

# ╔═╡ c78c961f-fccc-485b-93c3-c63f4d688121
probit6 = glm(@formula(is_restricted ~ age + education + gender_w + childcare_coverage_available + hourly_wage + wage_gap), design_matrix, Distributions.Binomial(), LogitLink())

# ╔═╡ 2b4c34b2-0b2c-48d6-a59e-e68596160884
probit7 = glm(@formula(downward_restricted ~ age + education + gender_w + hourly_wage + wage_gap), design_matrix, Distributions.Binomial(), LogitLink())

# ╔═╡ 08a6a932-4f37-49c8-9e5c-0be446cdb392
histogram(data[!, :probability_primary_child_care_responsibility_m])

# ╔═╡ cae2e482-975c-496c-ad60-35eff25a96b4
histogram(data[!, :hourly_wage_m])

# ╔═╡ 877432a5-20e3-4858-a7c8-62772fe44f36
histogram(data[!, :hourly_wage_w])

# ╔═╡ f085b251-3bae-4e47-bc32-33d04f3c28c2
@chain data begin
	@select(edu_age_to_wage(:education_m, :age_m))
end
...
[[deps.DistributionMeasures]]
deps = ["ArgCheck", "ArraysOfArrays", "ChainRulesCore", "ChangesOfVariables", "DensityInterface", "Distributions", "FillArrays", "ForwardDiff", "ForwardDiffPullbacks", "Functors", "InverseFunctions", "IrrationalConstants", "LinearAlgebra", "MeasureBase", "PDMats", "Random", "Static", "Statistics", "StatsBase", "StatsFuns"]
git-tree-sha1 = "67373e851133928ea68de8991554788915dde7e9"
...
[[deps.ForwardDiffPullbacks]]
deps = ["ChainRulesCore", "ConstructionBase", "ForwardDiff", "LinearAlgebra", "Static", "StaticArrays"]
git-tree-sha1 = "b2e2e9c81791aadbc379d0c58bc7a0f43c90476d"
...
git-tree-sha1 = "2391020282750b4fb0ddbffa734439f4d52c30db"
...
git-tree-sha1 = "b51d9a60aa2353936d637ae83efd615edebf63b3"
uuid = "fa1605e6-acd5-459c-a1e6-7e635759db14"
version = "0.13.1"

[[deps.MeasureTheory]]
deps = ["Accessors", "ChangesOfVariables", "Compat", "ConcreteStructs", "ConstructionBase", "DensityInterface", "DistributionMeasures", "Distributions", "DynamicIterators", "FillArrays", "IfElse", "Infinities", "InteractiveUtils", "InverseFunctions", "KeywordCalls", "LazyArrays", "LinearAlgebra", "LogExpFunctions", "MLStyle", "MacroTools", "MappedArrays", "MeasureBase", "NamedTupleTools", "PositiveFactorizations", "PrettyPrinting", "Random", "Reexport", "SpecialFunctions", "Static", "StaticArraysCore", "Statistics", "StatsBase", "StatsFuns", "TransformVariables", "Tricks"]
git-tree-sha1 = "ec2a70723259d0ebe23ad69664997637ecec8bfc"
...
git-tree-sha1 = "e087e0445249996f893f197582f5a69333e1e3eb"
...
git-tree-sha1 = "6429b7d3c3c03551ef501ac897df094b177305dc"
...
[[deps.Tilde]]
deps = ["Accessors", "ChainRulesCore", "DataStructures", "DensityInterface", "DistributionMeasures", "IfElse", "JuliaVariables", "LinearAlgebra", "MLStyle", "MacroTools", "MeasureBase", "MeasureTheory", "NamedTupleTools", "NestedTuples", "Random", "Reexport", "Requires", "SampleChains", "Serialization", "Static", "StatsBase", "StatsFuns", "TransformVariables", "Tricks", "TupleVectors"]
git-tree-sha1 = "4c11e39fc3687240596938960e4eabf581af41a1"
